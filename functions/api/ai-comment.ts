...
- 【トーンの使い分け】タスクごとに失敗したときの代償が異なります。畑しごと・施肥はやり直しが効く、または雨を味方にできるため、積極的に作業できる窓を探して攻めの提案をします。一方、農薬散布は流亡して効果を失うと貴重な「使用回数」を1回無駄に消費する重い代償があるため、確実性を重視して慎重な判断材料を提供してください。
...
- sprayingAdvice: 防除・散布のアドバイス（農薬や液肥などの散布が対象。農薬は流亡すると貴重な使用回数を浪費するため、確実性を重視して慎重に判断する）。作業できる時間帯は、散布後に薬剤が乾くまでの時間（晴天で概ね1〜2時間）が雨までに確保できるかを基準に、デッドライン（〇時頃まで）を示す。残るリスクは、予報の降水時刻・降水量や、風速3m/s以上での飛散・薬害など、データの数値を根拠に流亡リスクを伝える。軽減策は、薬剤の耐雨性をラベルで確認する示唆や、無理な場合の別日・別時間帯の代替案を添える。今日・明日・明後日の好機に加え数日先の天候崩れも見据える。最低130文字以上、250文字以内）
