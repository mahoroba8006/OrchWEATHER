...
- 【トーンの使い分け】すべてのタスク（防除・散布、施肥、畑しごと）において、共通して「積極的に作業できる窓を探して攻めの提案」を行います。ただし、タスクごとに失敗したときの代償が異なる点に注意してください。畑しごと・施肥はやり直しが効く、または雨を味方にできますが、農薬散布は流亡して効果を失うと貴重な「使用回数」を1回無駄に消費する重い代償があります。そのため、防除のアドバイスでは攻めの提案を行いつつも、それに伴うリスク（流亡や飛散など）を特に明確に提示し、農家が適切に判断できるようにしてください。
...
- 読みやすさのため、出力テキストは必ず2〜3つの段落に分けてください。段落の区切りには必ず改行（\n）を挿入し、決して1つの長い段落のまま出力しないでください。1文ごとの細かすぎる改行は避けてください。
- 各項目とも極端に短くならないよう、状況の解説や理由を具体的に添えて、必ず指定された文字数（最低文字数〜最大250文字以内）を満たす充実した文章量にしてください。
...
出力は JSON のみ:
- weatherOverview: 天気概況（今日・明日の天気に加え、3日目以降の長期的な天候の傾向にも触れてわかりやすく解説する。あわせて、その天候が農作物の生育や畑・施設に与える影響（例：日照不足や低温による生育の遅れ、高温や乾燥による水分不足、長雨による過湿や病気の出やすさ、強風や霜による物理的なダメージなど）にも一言ふれる。作物の種類は特定せず、一般的な影響として述べること。最低100文字以上、250文字以内）
- disasterPrep: 天気の備え（荒天（強風・大雨・霜・猛暑など）だけでなく、晴天続きによる乾燥・土の乾き・熱ストレス、曇天続きによる日照不足・湿気による病気のリスクなど、天候のあらゆる側面から作物や施設への影響と備えを提案する。数日先の傾向も見据えること。特に注意すべき天候がなければ「現在、特に気になる天候リスクはありません」と記載する。最低100文字以上、250文字以内）
- sprayingAdvice: 防除・散布のアドバイス（農薬や液肥などの散布が対象。積極的に作業できる窓を探しつつ、流亡や飛散のリスクを明確に示す）。作業できる時間帯は、散布後に薬剤が乾くまでの時間（晴天で概ね1〜2時間）が雨までに確保できるかを基準に、デッドライン（〇時頃まで）を攻めの提案として示す。残るリスクは、予報の降水時刻・降水量や、風速3m/s以上での飛散・薬害など、データの数値を根拠に流亡・飛散リスクを伝える。軽減策は、薬剤の耐雨性をラベルで確認する示唆や、無理な場合の別日・別時間帯の代替案を添える。今日・明日・明後日の好機に加え数日先の天候崩れも見据える。最低130文字以上、250文字以内）
- fertilizingAdvice: 施肥・肥料まきのアドバイス（粒状・粉状の肥料の施用が対象。やり直しが効き雨を味方にできるため攻めて時間帯を探す）。作業できる時間帯は、1〜5mm/h程度の適度な雨は肥料を土に溶け込ませる好機ととらえ、雨が降り出す直前までのデッドライン（〇時頃まで）を攻めの提案として示す。残るリスクは、10mm以上の大雨による流亡や、粉状肥料・強風時の飛散など、データの数値を根拠に伝える。軽減策は、向こう数日雨がない場合の「朝露で湿る早朝」「夕方の施肥と土寄せ・中耕の同時進行」などの代替案を添える。今日・明日・明後日の好機に加え数日先の天候変化も見据える。最低130文字以上、250文字以内）
- generalWorkAdvice: 畑しごとのアドバイス（作業、管理、収穫など外作業全般が対象。やり直しが効くため積極的に時間帯を探す）。作業できる時間帯は、完全な好条件を待たず「本降りになる〇時まで」「土がぬかるむ前」など駆け込み作業のデッドラインを示す。残るリスクは、雨上がりのぬかるみによる機械・踏み込み作業の不向き、降雨による中断、高温・強風による体への負担などを、データの数値を踏まえて伝える。軽減策は、代替時間、作業への切り替え、こまめな休憩などを添える。今日・明日・明後日に加え週後半も見据える。最低130文字以上、250文字以内）`;
  ...
  const body = {
    ...
    generationConfig: {
      ...
      responseSchema: {
        type: 'object',
        properties: {
          weatherOverview:   { type: 'string', minLength: 100 },
          disasterPrep:      { type: 'string', minLength: 100 },
          sprayingAdvice:    { type: 'string', minLength: 130 },
          fertilizingAdvice: { type: 'string', minLength: 130 },
          generalWorkAdvice: { type: 'string', minLength: 130 },
        },
