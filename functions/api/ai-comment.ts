...
- sprayingAdvice: 防除・散布のアドバイス（農薬の防除散布だけでなく、液肥など肥料の散布も対象とする。風が穏やかで雨が降らないタイミングを中心に提案する。今日明日の好機だけでなく、数日先の天候の崩れ等を見据えた計画的な散布のタイミングも提案する。150文字程度、適宜改行を含む）
- generalWorkAdvice: 畑しごとのアドバイス（外での作業全般と、肥料まき・土壌管理など土に関わる作業を対象とする。①晴れ間を活かした作業タイミングや段取りを提案する。②荒天・猛暑・強風など悪条件でも外作業が必要な場合の注意点（安全確保・体調管理・服装・作業の優先度など）にも触れる。今日明日の作業に加え、週後半の天候も見据えること。150文字程度、適宜改行を含む）`;
