...
- fertilizingAdvice: 施肥・肥料まきのアドバイス（固形肥料・液肥など肥料全般の施用タイミングを対象とする。以下の優先順で判断して提案すること。①最適枠（最優先）：散布の半日〜1日後に1〜5mm/h程度の弱い雨が降る見込みがある時間帯を最優先で提案する（適度な水分で肥料が土に溶け込む）。②晴天継続時：向こう数日雨がない場合は「朝露で土壌表面が湿っている早朝」または「夕方に撒いてすぐ中耕・土寄せで土中に混ぜ込む」を提案し、液肥の葉面散布への代替も提案する。③排除条件：10mm以上の大雨直前・風速3m/s以上の強風直前は流亡・飛散リスクがあるため避ける。数日先の天候の変化も見据えて計画的なタイミングを提案する。200文字程度、適宜改行を含む）
- generalWorkAdvice: 畑しごとのアドバイス（外での作業全般（草取り・収穫・定植・支柱立てなど）を対象とする。まず今日明日の天候を踏まえた一般的な外作業の好機・段取りを述べる。次に以下の順で掘り下げること。①雨と土の状態：機械作業・踏み込み作業は雨直後の軟化した土では不向き。長雨後の排水状況、雨前の駆け込み作業の可否も考慮する。②気温・体への負担：高温時は暑さ対策、早朝・夕方の作業を推奨。低温・強風時は防寒や作業時間の短縮にも言及する。③悪条件での外作業が必要な場合：安全確保・作業優先度の判断を提案する。今日明日の作業に加え、週後半の天候も見据えること。200文字程度、適宜改行を含む）`;
