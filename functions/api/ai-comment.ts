...
- 防除・散布／施肥／畑しごとのアドバイスは「作業できる窓・残るリスク・軽減策」の3点を必ず含めますが、①②③などの番号やラベルは出力に書かず、自然な文章として織り込んでください。
...
- 【重要】読みやすさのため、出力テキストは必ず2〜3つの段落に分けてください。段落の区切りには必ず改行（\n）を挿入し、決して1つの長い段落のまま出力しないでください。1文ごとの細かすぎる改行は避けてください。
...
出力は JSON のみ:
- weatherOverview: 天気概況（今日・明日の天気に加え、3日目以降の長期的な天候の傾向にも触れてわかりやすく解説する。あわせて、その天候が農作物の生育や畑・施設に与える影響（例：日照不足や低温による生育の遅れ、高温や乾燥による水分不足、長雨による過湿や病気の出やすさ、強風や霜による物理的なダメージなど）にも一言ふれる。作物の種類は特定せず、一般的な影響として述べること。150文字程度。2〜3段落で構成し、段落間に改行（\n）を入れること。）
- disasterPrep: 天気の備え（荒天（強風・大雨・霜・猛暑など）だけでなく、晴天続きによる乾燥・土の乾き・熱ストレス、曇天続きによる日照不足・湿気による病気のリスクなど、天候のあらゆる側面から作物や施設への影響と備えを提案する。数日先の傾向も見据えること。特に注意すべき天候がなければ「現在、特に気になる天候リスクはありません」と記載する。150文字程度。2〜3段落で構成し、段落間に改行（\n）を入れること。）
- sprayingAdvice: 防除・散布のアドバイス（農薬や液肥などの散布が対象。農薬は流亡すると貴重な使用回数を浪費するため、確実性を重視して慎重に判断する）。「作業できる窓・残るリスク・軽減策」を自然な文章で織り込むこと。作業できる窓は、散布後に薬剤が乾くまでの時間（晴天で概ね1〜2時間）が雨までに確保できるかを基準に、デッドライン（〇時頃まで）を示す。残るリスクは、予報の降水時刻・降水量や、風速3m/s以上での飛散・薬害など、データの数値を根拠に流亡リスクを伝える。軽減策は、薬剤の耐雨性をラベルで確認する示唆や、無理な場合の別日・別時間帯の代替案を添える。今日明日の好機に加え数日先の天候崩れも見据える。180文字程度。2〜3段落で構成し、段落間に改行（\n）を入れること。）
- fertilizingAdvice: 施肥・肥料まきのアドバイス（粒状・粉状の肥料の施用が対象。やり直しが効き雨を味方にできるため攻めて窓を探す）。「作業できる窓・残るリスク・軽減策」を自然な文章で織り込むこと。作業できる窓は、1〜5mm/h程度の適度な雨は肥料を土に溶け込ませる好機ととらえ、雨が降り出す直前までのデッドライン（〇時頃まで）を攻めの提案として示す。残るリスクは、10mm以上の大雨による流亡や、粉状肥料・強風時の飛散など、データの数値を根拠に伝える。軽減策は、向こう数日雨がない場合の「朝露で湿る早朝」「夕方の施肥と土寄せ・中耕の同時進行」などの代替案を添える。数日先の天候変化も見据える。180文字程度。2〜3段落で構成し、段落間に改行（\n）を入れること。）
- generalWorkAdvice: 畑しごとのアドバイス（収穫・除草・管理など外作業全般が対象。やり直しが効くため積極的に窓を探す）。「作業できる窓・残るリスク・軽減策」を自然な文章で織り込むこと。作業できる窓は、完全な好条件を待たず「本降りになる〇時まで」「土がぬかるむ前」など駆け込み作業のデッドラインを示す。残るリスクは、雨上がりのぬかるみによる機械・踏み込み作業の不向き、降雨による中断、高温・強風による体への負担などを、データの数値を踏まえて伝える。軽減策は、風の影響を受けにくい作業への切り替え、こまめな休憩、代替時間などを添える。今日明日に加え週後半も見据える。180文字程度。2〜3段落で構成し、段落間に改行（\n）を入れること。）`;
