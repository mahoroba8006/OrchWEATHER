...
出力は JSON のみ:
- weatherOverview: 天気概況（今日・明日の天気に加え、3日目以降の長期的な天候の傾向にも触れてわかりやすく解説する。あわせて、その天候が農作物の生育や畑・施設に与える影響（例：日照不足や低温による生育の遅れ、高温や乾燥による水分不足、長雨による過湿や病気の出やすさ、強風や霜による物理的なダメージなど）にも一言ふれる。作物の種類は特定せず、一般的な影響として述べること。150文字程度。文脈の区切りで適切に改行。）
- disasterPrep: 天気の備え（荒天（強風・大雨・霜・猛暑など）だけでなく、晴天続きによる乾燥・土の乾き・熱ストレス、曇天続きによる日照不足・湿気による病気のリスクなど、天候のあらゆる側面から作物や施設への影響と備えを提案する。数日先の傾向も見据えること。特に注意すべき天候がなければ「現在、特に気になる天候リスクはありません」と記載する。150文字程度。文脈の区切りで適切に改行。）
- sprayingAdvice: 防除・散布のアドバイス（農薬の防除散布だけでなく、液肥など肥料の散布も対象とする。以下の優先順で判断して提案すること。①降雨リスク（最優先）：散布後、最低3時間（理想は4〜6時間）以内に1mm以上の降雨がない時間帯を最優先で提案する。降水確率だけでなく降水量の見込みも考慮する。②風の状況：穏やかで風向きが安定しているほど好条件。強い・不安定な風は飛散リスクがある。③気温と時間帯：散布液の蒸散を抑えるため、早朝から午前中早めの時間が望ましい。今日明日の好機と、数日先の天候崩れを見据えた計画的なタイミングを合わせて提案する。150文字程度。文脈の区切りで適切に改行。）
- fertilizingAdvice: 施肥・肥料まきのアドバイス（粒状・粉状の肥料の施用タイミングを対象とする。以下の優先順で判断して提案すること。①最適枠（最優先）：散布の半日〜1日後に1〜5mm/h程度の弱い雨が降る見込みがある時間帯を最優先で提案する（適度な水分で肥料が土に溶け込む）。②晴天継続時：向こう数日雨がない場合は「朝露で土壌表面が湿っている早朝」または「夕方に撒いてすぐ中耕・土寄せで土中に混ぜ込む」などの代替案を提案する。③排除条件：大雨直前（10mm程度以上）、または、強風直前（風速3m/s程度以上）のは流亡・飛散リスクがあるため避ける。数日先の天候の変化も見据えて計画的なタイミングを提案する。150文字程度。文脈の区切りで適切に改行。）
- generalWorkAdvice: 畑しごとのアドバイス（外での作業全般を対象とする。まず今日明日の天候を踏まえた一般的な外作業の好機・段取りを述べる。次に以下の順で掘り下げること。①雨と土の状態：機械作業・踏み込み作業は雨直後の軟化した土では不向き。長雨後の排水状況、雨前の駆け込み作業の可否も考慮する。②気温・体への負担：高温時は暑さ対策、早朝・夕方の作業を推奨。低温・強風時は防寒や作業時間の短縮にも言及する。③悪条件での外作業が必要な場合：安全確保・作業優先度の判断を提案する。今日明日の作業に加え、週後半の天候も見据えること。150文字程度。文脈の区切りで適切に改行。）`;
