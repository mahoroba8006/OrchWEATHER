...
- 改行（\\n）は、日付が変わるタイミングや話題が大きく変わる箇所にのみ入れてください。一文の途中や、同じ日の続きの話では改行しないでください。
...
出力は JSON のみ:
- weatherOverview: 天気概況（今日・明日の天気に加え、3日目以降の長期的な天候の傾向にも触れてわかりやすく解説する。150文字程度、適宜改行を含む）
- disasterPrep: 悪天候への備え（強風・大雨・霜・猛暑など作物や施設に影響するリスクを注意喚起する。数日先の荒天リスクも見据えること。リスクがなければ「現在、特に注意すべき悪天候の予報はありません」と記載する。150文字程度、適宜改行を含む）
