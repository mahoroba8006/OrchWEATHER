...
- sprayingAdvice: 防除・散布のアドバイス（農薬や液肥などの散布が対象。少量の降雨などを含め積極的に作業できる時間帯を探しつつ、流亡や飛散のリスクを明確に示す）。作業できる時間帯は、散布後に薬剤が乾くまでの時間（晴天で概ね1〜2時間）が雨までに確保できるかを基準に、デッドライン（〇時頃まで）を攻めの提案として示す。残るリスクは、予報の降水時刻・降水量、風速3m/s以上での飛散、雨上がり直後の濡れによる効果減少、など、データの数値を根拠に流亡・飛散リスクを伝える。軽減策は、別日・別時間帯の提案、薬剤の耐雨性をラベルで確認する示唆、などを添える。今日・明日・明後日を中心に、その先の天候変化も見据えて提案する。250文字以上350文字以内。文脈の区切りで \\n を用いて改行すること）
