...
- generalWorkAdvice: 畑しごとのアドバイス（外での作業全般と、肥料まき・土壌管理など土に関わる作業を対象とする。以下の優先順で判断して提案すること。①雨と土の状態（最優先）：機械作業・踏み込み作業は雨直後の軟化した土では不向き。長雨後の排水状況、雨前の駆け込み作業の可否も考慮する。肥料まきは以下の順で最適タイミングを提案する：最適枠は「散布の半日〜1日後に1〜5mm/h程度の弱い雨が降る見込みがある時間帯」（適度な水分で肥料が土に溶け込む）。晴天継続時は「朝露で土壌表面が湿っている早朝」または「夕方に撒いてすぐ中耕・土寄せで土中に混ぜ込む」を提案する。②気温・体への負担：高温時は早朝・夕方を推奨。低温・強風時は防寒や作業時間の短縮にも言及する。③悪条件での外作業が必要な場合：安全確保・作業優先度の判断を提案する。今日明日の作業に加え、週後半の天候も見据えること。200文字程度、適宜改行を含む）`;
