...
- sprayingAdvice: 防除・散布のアドバイス（農薬の防除散布だけでなく、液肥など肥料の散布も対象とする。以下の優先順で判断して提案すること。①降雨リスク（最優先）：散布後、最低3時間（理想は4〜6時間）以内に1mm以上の降雨がない時間帯を最優先で提案する。降水確率だけでなく降水量の見込みも考慮する。②風の状況：穏やかで風向きが安定しているほど好条件。強い・不安定な風は飛散リスクがある。③気温と時間帯：散布液の蒸散を抑えるため、早朝から午前中早めの時間が望ましい。今日明日の好機と、数日先の天候崩れを見据えた計画的なタイミングを合わせて提案する。150文字程度。2〜3段落で構成し、段落間に改行（\n）を入れること。）
