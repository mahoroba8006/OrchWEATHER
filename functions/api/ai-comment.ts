...
出力は JSON のみ:
- weatherOverview: 天気概況（今日・明日の天気に加え、3日目以降の長期的な天候の傾向にも触れてわかりやすく解説する。あわせて、その天候が農作物の生育や畑・施設に与える影響（例：日照不足や低温による生育の遅れ、高温や乾燥による水分不足、長雨による過湿や病気の出やすさ、強風や霜による物理的なダメージなど）にも一言ふれる。作物の種類は特定せず、一般的な影響として述べること。最低100文字以上、250文字以内。文脈の区切りで \\n を用いて改行すること）
- disasterPrep: 天気の備え（荒天（強風・大雨・霜・猛暑など）だけでなく、晴天続きによる乾燥・土の乾き・熱ストレス、曇天続きによる日照不足・湿気による病気のリスクなど、天候のあらゆる側面から作物や施設への影響と備えを提案する。数日先の傾向も見据えること。特に注意すべき天候がなければ「現在、特に気になる天候リスクはありません」と記載する。最低100文字以上、250文字以内。文脈の区切りで \\n を用いて改行すること）
...
- generalWorkAdvice: 畑しごとのアドバイス（作業、管理、収穫など外作業全般が対象。やり直しが効くため積極的に時間帯を探す）。作業できる時間帯は、完全な好条件を待たず「本降りになる〇時まで」「土がぬかるむ前」など駆け込み作業のデッドラインを示す。残るリスクは、雨上がりのぬかるみによる機械・踏み込み作業の不向き、降雨による中断、高温・強風による体への負担などを、データの数値を踏まえて伝える。軽減策は、代替時間、作業への切り替え、こまめな休憩などを添える。今日・明日・明後日に加え週後半も見据える。最低130文字以上、250文字以内。文脈の区切りで \\n を用いて改行すること）`;
