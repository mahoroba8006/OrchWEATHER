...
| リスク | カスタマイズ方式 | 条件式（時間別） | 備考 |
|--------|----------------|----------------|------|
| 霜     | 数値入力×2 | 気温 ≤ X **＆** 露点 ≤ Y | 日別は気温のみ（露点データなし） |
| 強風   | 数値入力 | 風速 ≥ X | 時間別・日別で同一閾値 |
| 大雨   | 数値入力×2 | 時間雨量 ≥ X **＆** 日雨量 ≥ Y | 夕立型と長雨型を区別 |
| 高温   | 数値入力 | 気温 ≥ X | 時間別・日別で同一閾値 |
| 乾燥   | 数値入力 | 湿度 ≤ X | 時間別・日別で同一閾値 |
| 雷雨   | 感度スライダー（控えめ/標準/敏感） | CAPE ≥ X | CAPE閾値に内部マッピング |
| 雹     | 感度スライダー＋数値入力 | CAPE ≥ X **＆** 0℃層高度 ≤ Y | 日別はWMOコード固定（freezingLevel なし） |

**日別予報の雷雨・雹（WMOコード 95〜99）は変更なし。**
`frostDewPoint`・`hailFreezingLevel` は時間別判定のみに効く。
...
export interface RiskThresholds {
  ...
  thunderSensitivity: RiskSensitivity; // 雷雨感度（CAPE）            デフォルト: 'medium'
  hailSensitivity:    RiskSensitivity; // 雹感度（CAPE）              デフォルト: 'medium'
...
| 項目                     | 最小      | 最大      | ステップ | 適用レイヤー |
|------------------------|----------|----------|---------|------------|
| 霜 気温                  | −5 ℃     | 5 ℃      | 0.5     | 時間別・日別 |
| 霜 露点温度 ＆             | −5 ℃     | 3 ℃      | 0.5     | 時間別のみ  |
| 強風 風速                 | 5 m/s    | 30 m/s   | 1       | 時間別・日別 |
| 大雨 時間雨量              | 10 mm/h  | 100 mm/h | 5       | 時間別のみ  |
| 大雨 日雨量               | 20 mm    | 300 mm   | 10      | 日別のみ    |
| 高温 気温                 | 28 ℃     | 42 ℃     | 0.5     | 時間別・日別 |
| 乾燥 湿度                 | 10 %     | 60 %     | 5       | 時間別・日別 |
| 雹 0℃層高度 ＆            | 2000 m   | 5000 m   | 100     | 時間別のみ  |
...
| 判定 | 変更前 | 変更後 |
|------|--------|--------|
| 霜（時間別） | `h.dewPoint <= 0 && h.temperature <= 3` | `h.dewPoint <= t.frostDewPoint && h.temperature <= t.frost` |
| 雷雨（時間別） | `h.cape >= 500` | `h.cape >= thunderCape` |
| 雹（時間別） | `h.cape >= 1000 && h.freezingLevel <= 3500` | `h.cape >= hailCape && h.freezingLevel <= t.hailFreezingLevel` |
...
1つの `glass-panel` に全設定をまとめる。＆条件は視覚的に「＆」で接続して複合条件であることを明示する。

```
┌─ glass-panel ──────────────────────────────────────────────────┐
│ h3: リスク検知の閾値                                              │
│                                                                 │
│  霜  気温 ≤ [ 3.0 ] ℃   ＆   露点 ≤ [ 0.0 ] ℃  ← 1行に並べる  │
│                                                                 │
│  ┌──────────────────┐  ┌───────────────────┐                   │
│  │ 強風 風速 ≥        │  │ 高温 気温 ≥         │                   │
│  │ [ 15  ] m/s      │  │ [ 35.0  ] ℃       │                   │
