...
`buildValidPeriodMap` で `〜` の左側を `Date.parse()` で変換し `startMs` として保持する。**既存の `endMs` 算出と完全に同じ年補完ロジックを使用すること**（12月末〜1月にまたがるケースで年が1つずれる問題を既存実装が対処済みであれば、同じ関数を流用する）。
 ...
 * endMs が undefined の場合は Infinity として扱い、そのレーンは後続の警報を受け付けない。
 */
...
**解除未定の扱い（重要）:** `endMs === undefined` の警報が配置されたレーンは終端を `Infinity` として管理し、後続の警報が同じレーンに混入しないようにする。
...
| 4日目以降 | `[00:00, 翌00:00)` | 該当日と重なれば対象（日全体） |

時間粒度は「列単位」（AM/PM/夜間列内の細かい位置は不要）。

**境界値の扱い:** 各列の時間範囲は **左閉・右開区間** `[start, end)` として判定する。例えば `endMs` がちょうど 12:00:00 の場合、午前列のみに属し午後列には属さない。気象庁の発表が正時区切りであっても表示上の重複を防げる。
...
HourlyTable には左端に行ラベルの固定列が存在する。ガントバー行もこれに合わせ、**空の `<td>`（ラベルセル）＋ `<td colSpan={hourCols}>`（バー領域）** の構造にする。

```tsx
{lanes.map((lane, i) => (
  <tr key={i} className="gantt-row">
    <td /> {/* 左端ラベル列と高さを揃えるための空セル */}
    <td colSpan={hourCols} style={{ position: 'relative', height: 22 }}>
      {lane.map(warning => (
        <WarningBar
          key={warning.code}
          warning={warning}
