# 天気SVGアイコン統合 実装プラン

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 天気表示を Unicode 絵文字から Climacons SVG アイコン（色付き）に置き換える

**Architecture:** Climacons SVG は全て単一 `<path>` 要素で構成されているため、`d` 属性値を TypeScript 定数として `WeatherIcon.tsx` に埋め込み、`<svg><path fill={color} />` として描画する。新パッケージ不要。夜間切り替えは WMO code 0（快晴→Moon）と code 1–2（薄曇り→Cloud-Moon）のみ行い、それ以外は昼用アイコンを使用する。

**Tech Stack:** React 19, TypeScript, Vite

---

## ファイルマップ

| 操作 | パス | 内容 |
|------|------|------|
| 新規作成 | `src/components/weather/WeatherIcon.tsx` | SVGパスデータ定数 + WeatherIcon コンポーネント |
| 修正 | `src/components/weather/HourlyTable.tsx` | nightWeatherNode 削除、WeatherIcon に置換 |
| 修正 | `src/components/weather/DailyForecast.tsx` | weatherCodeToEmoji を WeatherIcon に置換 |

---

## Task 1: WeatherIcon コンポーネントの作成

**Files:**
- Create: `src/components/weather/WeatherIcon.tsx`

- [ ] **Step 1: ファイルを作成する**

`src/components/weather/WeatherIcon.tsx` を以下の内容で作成する。

```tsx
// src/components/weather/WeatherIcon.tsx

const PATHS: Record<string, string> = {
  Sun: 'M71.997,51.999h-3.998c-1.105,0-2-0.895-2-1.999s0.895-2,2-2h3.998c1.105,0,2,0.896,2,2S73.103,51.999,71.997,51.999z M64.142,38.688c-0.781,0.781-2.049,0.781-2.828,0c-0.781-0.781-0.781-2.047,0-2.828l2.828-2.828c0.779-0.781,2.047-0.781,2.828,0c0.779,0.781,0.779,2.047,0,2.828L64.142,38.688z M50.001,61.998c-6.627,0-12-5.372-12-11.998c0-6.627,5.372-11.999,12-11.999c6.627,0,11.998,5.372,11.998,11.999C61.999,56.626,56.628,61.998,50.001,61.998z M50.001,42.001c-4.418,0-8,3.581-8,7.999c0,4.417,3.583,7.999,8,7.999s7.998-3.582,7.998-7.999C57.999,45.582,54.419,42.001,50.001,42.001z M50.001,34.002c-1.105,0-2-0.896-2-2v-3.999c0-1.104,0.895-2,2-2c1.104,0,2,0.896,2,2v3.999C52.001,33.106,51.104,34.002,50.001,34.002z M35.86,38.688l-2.828-2.828c-0.781-0.781-0.781-2.047,0-2.828s2.047-0.781,2.828,0l2.828,2.828c0.781,0.781,0.781,2.047,0,2.828S36.641,39.469,35.86,38.688z M34.002,50c0,1.104-0.896,1.999-2,1.999h-4c-1.104,0-1.999-0.895-1.999-1.999s0.896-2,1.999-2h4C33.107,48,34.002,48.896,34.002,50z M35.86,61.312c0.781-0.78,2.047-0.78,2.828,0c0.781,0.781,0.781,2.048,0,2.828l-2.828,2.828c-0.781,0.781-2.047,0.781-2.828,0c-0.781-0.78-0.781-2.047,0-2.828L35.86,61.312z M50.001,65.998c1.104,0,2,0.895,2,1.999v4c0,1.104-0.896,2-2,2c-1.105,0-2-0.896-2-2v-4C48.001,66.893,48.896,65.998,50.001,65.998z M64.142,61.312l2.828,2.828c0.779,0.781,0.779,2.048,0,2.828c-0.781,0.781-2.049,0.781-2.828,0l-2.828-2.828c-0.781-0.78-0.781-2.047,0-2.828C62.093,60.531,63.36,60.531,64.142,61.312z',

  Moon: 'M50,61.998c-6.627,0-11.999-5.372-11.999-11.998c0-6.627,5.372-11.999,11.999-11.999c0.755,0,1.491,0.078,2.207,0.212c-0.132,0.576-0.208,1.173-0.208,1.788c0,4.418,3.582,7.999,8,7.999c0.615,0,1.212-0.076,1.788-0.208c0.133,0.717,0.211,1.452,0.211,2.208C61.998,56.626,56.626,61.998,50,61.998z M48.212,42.208c-3.556,0.813-6.211,3.989-6.211,7.792c0,4.417,3.581,7.999,7.999,7.999c3.802,0,6.978-2.655,7.791-6.211C52.937,50.884,49.115,47.062,48.212,42.208z',

  CloudSun: 'M79.941,43.641h-4c-1.104,0-2-0.895-2-2c0-1.104,0.896-1.998,2-1.998h4c1.104,0,2,0.895,2,1.998C81.941,42.746,81.045,43.641,79.941,43.641z M72.084,30.329c-0.781,0.781-2.047,0.781-2.828,0c-0.781-0.78-0.781-2.047,0-2.827l2.828-2.828c0.781-0.781,2.047-0.781,2.828,0c0.781,0.78,0.781,2.047,0,2.828L72.084,30.329z M69.137,45.936L69.137,45.936c1.749,2.086,2.806,4.77,2.806,7.705c0,6.625-5.372,11.998-11.999,11.998c-2.775,0-12.801,0-15.998,0c-8.835,0-15.998-7.162-15.998-15.998s7.163-15.998,15.998-15.998c1.572,0,3.09,0.232,4.523,0.654c2.195-2.827,5.618-4.654,9.475-4.654c6.627,0,11.999,5.373,11.999,11.998C69.942,43.156,69.649,44.602,69.137,45.936z M31.947,49.641c0,6.627,5.371,11.998,11.998,11.998c3.616,0,12.979,0,15.998,0c4.418,0,7.999-3.582,7.999-7.998c0-4.418-3.581-8-7.999-8c-1.6,0-3.083,0.482-4.333,1.291c-1.231-5.316-5.974-9.289-11.665-9.289C37.318,37.643,31.947,43.014,31.947,49.641z M57.943,33.643c-2.212,0-4.215,0.898-5.662,2.349c2.34,1.436,4.285,3.453,5.629,5.854c0.664-0.113,1.337-0.205,2.033-0.205c2.125,0,4.119,0.559,5.85,1.527l0,0c0.096-0.494,0.15-1.004,0.15-1.527C65.943,37.225,62.361,33.643,57.943,33.643z M57.943,25.643c-1.104,0-1.999-0.895-1.999-1.999v-3.999c0-1.105,0.896-2,1.999-2c1.105,0,2,0.895,2,2v3.999C59.943,24.749,59.049,25.643,57.943,25.643z M43.803,30.329l-2.827-2.827c-0.781-0.781-0.781-2.048,0-2.828c0.78-0.781,2.047-0.781,2.827,0l2.828,2.828c0.781,0.78,0.781,2.047,0,2.827C45.851,31.11,44.584,31.11,43.803,30.329z',

  CloudMoon: 'M69.763,46.758L69.763,46.758c1.368,1.949,2.179,4.318,2.179,6.883c0,6.625-5.371,11.998-11.998,11.998c-2.775,0-12.801,0-15.998,0c-8.836,0-15.998-7.162-15.998-15.998s7.162-15.998,15.998-15.998c2.002,0,3.914,0.375,5.68,1.047l0,0c1.635-4.682,6.078-8.047,11.318-8.047c0.755,0,1.491,0.078,2.207,0.212c-0.131,0.575-0.207,1.173-0.207,1.788c0,4.418,3.581,7.999,7.998,7.999c0.616,0,1.213-0.076,1.789-0.208c0.133,0.717,0.211,1.453,0.211,2.208C72.941,41.775,71.73,44.621,69.763,46.758z M31.947,49.641c0,6.627,5.371,11.998,11.998,11.998c3.616,0,12.979,0,15.998,0c4.418,0,7.999-3.582,7.999-7.998c0-4.418-3.581-8-7.999-8c-1.6,0-3.083,0.482-4.334,1.291c-1.231-5.316-5.973-9.29-11.664-9.29C37.318,37.642,31.947,43.014,31.947,49.641z M51.496,35.545c0.001,0,0.002,0,0.002,0S51.497,35.545,51.496,35.545z M59.155,30.85c-2.9,0.664-5.175,2.91-5.925,5.775l0,0c1.918,1.372,3.523,3.152,4.68,5.22c0.664-0.113,1.337-0.205,2.033-0.205c2.618,0,5.033,0.85,7.005,2.271l0,0c0.858-0.979,1.485-2.168,1.786-3.482C63.881,39.525,60.059,35.706,59.155,30.85z',

  Cloud: 'M43.945,65.639c-8.835,0-15.998-7.162-15.998-15.998c0-8.836,7.163-15.998,15.998-15.998c6.004,0,11.229,3.312,13.965,8.203c0.664-0.113,1.338-0.205,2.033-0.205c6.627,0,11.999,5.373,11.999,12c0,6.625-5.372,11.998-11.999,11.998C57.168,65.639,47.143,65.639,43.945,65.639z M59.943,61.639c4.418,0,8-3.582,8-7.998c0-4.418-3.582-8-8-8c-1.6,0-3.082,0.481-4.333,1.291c-1.231-5.316-5.974-9.29-11.665-9.29c-6.626,0-11.998,5.372-11.998,11.999c0,6.626,5.372,11.998,11.998,11.998C47.562,61.639,56.924,61.639,59.943,61.639z',

  CloudFog: 'M69.998,65.641H30.003c-1.104,0-2-0.896-2-2c0-1.105,0.896-2,2-2h39.995c1.104,0,2,0.895,2,2C71.998,64.744,71.103,65.641,69.998,65.641z M69.998,57.641H30.003c-1.104,0-2-0.895-2-2c0-1.104,0.896-2,2-2h39.995c1.104,0,2,0.896,2,2C71.998,56.746,71.103,57.641,69.998,57.641z M59.999,45.643c-1.601,0-3.083,0.48-4.333,1.291c-1.232-5.317-5.974-9.291-11.665-9.291c-6.626,0-11.998,5.373-11.998,12h-4c0-8.835,7.163-15.999,15.998-15.999c6.004,0,11.229,3.312,13.965,8.204c0.664-0.113,1.337-0.205,2.033-0.205c5.222,0,9.652,3.342,11.301,8h-4.381C65.535,47.253,62.958,45.643,59.999,45.643z M30.003,69.639h39.995c1.104,0,2,0.896,2,2c0,1.105-0.896,2-2,2H30.003c-1.104,0-2-0.895-2-2C28.003,70.535,28.898,69.639,30.003,69.639z',

  CloudDrizzle: 'M63.943,64.941v-4.381c2.387-1.385,3.998-3.961,3.998-6.92c0-4.418-3.58-8-7.998-8c-1.602,0-3.084,0.481-4.334,1.291c-1.232-5.316-5.973-9.29-11.664-9.29c-6.627,0-11.998,5.372-11.998,11.999c0,3.549,1.549,6.729,3.998,8.926v4.914c-4.777-2.768-7.998-7.922-7.998-13.84c0-8.836,7.162-15.998,15.998-15.998c6.004,0,11.229,3.312,13.965,8.203c0.664-0.113,1.336-0.205,2.033-0.205c6.627,0,11.998,5.373,11.998,12C71.941,58.861,68.6,63.293,63.943,64.941z M41.945,53.641c1.104,0,2,0.895,2,2v3.998c0,1.105-0.896,2-2,2c-1.105,0-2-0.895-2-2v-3.998C39.945,54.535,40.84,53.641,41.945,53.641z M41.945,65.639c1.104,0,2,0.895,2,2v3.998c0,1.105-0.896,2-2,2c-1.105,0-2-0.895-2-2v-3.998C39.945,66.533,40.84,65.639,41.945,65.639z M49.943,57.639c1.105,0,2,0.896,2,2v4c0,1.104-0.895,2-2,2c-1.104,0-1.998-0.896-1.998-2v-4C47.945,58.535,48.84,57.639,49.943,57.639z M49.943,69.639c1.105,0,2,0.895,2,1.998v4c0,1.105-0.895,2-2,2c-1.104,0-1.998-0.895-1.998-2v-4C47.945,70.533,48.84,69.639,49.943,69.639z M57.943,53.641c1.104,0,2,0.895,2,2v3.998c0,1.105-0.896,2-2,2c-1.105,0-2-0.895-2-2v-3.998C55.943,54.535,56.838,53.641,57.943,53.641z M57.943,65.639c1.104,0,2,0.895,2,2v3.998c0,1.105-0.896,2-2,2c-1.105,0-2-0.895-2-2v-3.998C55.943,66.533,56.838,65.639,57.943,65.639z',

  CloudRain: 'M63.943,64.941v-4.381c2.389-1.383,4-3.961,4-6.92c0-4.417-3.582-7.999-8-7.999c-1.6,0-3.082,0.48-4.333,1.291c-1.231-5.317-5.974-9.29-11.665-9.29c-6.626,0-11.998,5.372-11.998,11.998c0,3.55,1.551,6.728,4,8.925v4.916c-4.777-2.768-8-7.922-8-13.841c0-8.835,7.163-15.997,15.998-15.997c6.004,0,11.229,3.311,13.965,8.203c0.664-0.113,1.338-0.205,2.033-0.205c6.627,0,11.999,5.372,11.999,11.999C71.942,58.863,68.601,63.293,63.943,64.941z M41.946,53.641c1.104,0,1.999,0.896,1.999,2v15.998c0,1.105-0.895,2-1.999,2s-2-0.895-2-2V55.641C39.946,54.537,40.842,53.641,41.946,53.641z M49.945,57.641c1.104,0,2,0.895,2,2v15.998c0,1.104-0.896,2-2,2s-2-0.896-2-2V59.641C47.945,58.535,48.841,57.641,49.945,57.641z M57.944,53.641c1.104,0,1.999,0.896,1.999,2v15.998c0,1.105-0.895,2-1.999,2s-2-0.895-2-2V55.641C55.944,54.537,56.84,53.641,57.944,53.641z',

  CloudHail: 'M63.999,64.941v-4.381c2.389-1.383,3.999-3.961,3.999-6.92c0-4.417-3.581-7.999-7.998-7.999c-1.601,0-3.084,0.48-4.334,1.291c-1.231-5.317-5.974-9.29-11.665-9.29c-6.626,0-11.998,5.372-11.998,11.998c0,3.55,1.55,6.728,3.999,8.925v4.916c-4.776-2.768-7.998-7.922-7.998-13.841c0-8.835,7.162-15.997,15.997-15.997c6.004,0,11.229,3.311,13.966,8.203c0.663-0.113,1.336-0.205,2.033-0.205c6.626,0,11.998,5.372,11.998,11.999C71.998,58.863,68.656,63.293,63.999,64.941z M42.002,65.639c-1.104,0-1-0.895-1-1.998v-8c0-1.104-0.104-2,1-2s1,0.896,1,2v8C43.002,64.744,43.106,65.639,42.002,65.639z M42.002,69.639c1.104,0,1.999,0.896,1.999,2c0,1.105-0.895,2-1.999,2s-2-0.895-2-2C40.002,70.535,40.897,69.639,42.002,69.639z M50.001,69.639c-1.104,0-1-0.895-1-2v-7.998c0-1.105-0.104-2,1-2s1,0.895,1,2v7.998C51.001,68.744,51.105,69.639,50.001,69.639z M50.001,73.639c1.104,0,1.999,0.895,1.999,2c0,1.104-0.895,2-1.999,2s-2-0.896-2-2C48.001,74.533,48.896,73.639,50.001,73.639z M58,65.639c-1.104,0-1-0.895-1-1.998v-8c0-1.104-0.104-2,1-2s1,0.896,1,2v8C59,64.744,59.104,65.639,58,65.639z M58,69.639c1.104,0,2,0.896,2,2c0,1.105-0.896,2-2,2s-2-0.895-2-2C56,70.535,56.896,69.639,58,69.639z',

  Snowflake: 'M59.659,46.733l-1.958,1.13c0.188,0.682,0.298,1.396,0.298,2.137c0,0.742-0.109,1.456-0.298,2.138l1.958,1.13c0.956,0.553,1.284,1.775,0.731,2.732c-0.552,0.956-1.774,1.284-2.731,0.731l-1.954-1.128c-1.003,1.02-2.277,1.766-3.705,2.134v2.262c0,1.104-0.896,2-2,2s-2-0.896-2-2v-2.262c-1.428-0.368-2.703-1.114-3.705-2.134l-1.954,1.128c-0.957,0.553-2.18,0.225-2.731-0.731c-0.553-0.957-0.225-2.18,0.731-2.732l1.958-1.13c-0.189-0.682-0.298-1.396-0.298-2.138c0-0.741,0.108-1.455,0.298-2.137l-1.958-1.13c-0.956-0.553-1.284-1.775-0.731-2.732c0.552-0.956,1.774-1.284,2.731-0.731l1.954,1.128c1.002-1.02,2.277-1.766,3.705-2.134v-2.262c0-1.104,0.896-2,2-2s2,0.896,2,2v2.262c1.428,0.368,2.702,1.114,3.705,2.134l1.954-1.128c0.957-0.553,2.18-0.225,2.731,0.731C60.943,44.958,60.615,46.181,59.659,46.733z M50,46.001c-2.209,0-4,1.791-4,3.999c0,2.209,1.791,4,4,4s3.999-1.791,3.999-4C53.999,47.792,52.209,46.001,50,46.001z',

  CloudLightning: 'M59.999,65.641c-0.28,0-0.649,0-1.062,0l3.585-4.412c3.181-1.057,5.477-4.053,5.477-7.588c0-4.418-3.581-7.998-7.999-7.998c-1.601,0-3.083,0.48-4.333,1.29c-1.232-5.316-5.974-9.29-11.665-9.29c-6.626,0-11.998,5.372-11.998,12c0,5.446,3.632,10.038,8.604,11.504l-1.349,3.777c-6.52-2.021-11.255-8.098-11.255-15.282c0-8.835,7.163-15.999,15.998-15.999c6.004,0,11.229,3.312,13.965,8.204c0.664-0.114,1.337-0.205,2.033-0.205c6.627,0,11.999,5.371,11.999,11.998S66.626,65.641,59.999,65.641z M48.001,51.641h9.998l-5.999,10h6.999L46.001,77.639l3.6-11.998h-6.6L48.001,51.641z',
};

const COLORS: Record<string, string> = {
  Sun:           '#F59E0B',
  Moon:          '#94A3B8',
  CloudSun:      '#FBBF24',
  CloudMoon:     '#A0AEC0',
  Cloud:         '#9CA3AF',
  CloudFog:      '#B0B5C4',
  CloudDrizzle:  '#60A5FA',
  CloudRain:     '#3B82F6',
  CloudHail:     '#06B6D4',
  Snowflake:     '#93C5FD',
  CloudLightning:'#F87171',
};

function codeToIconName(code: number, isNight: boolean): string {
  if (code === 0)                           return isNight ? 'Moon'       : 'Sun';
  if (code <= 2)                            return isNight ? 'CloudMoon'  : 'CloudSun';
  if (code === 3)                           return 'Cloud';
  if (code === 45 || code === 48)           return 'CloudFog';
  if (code >= 51 && code <= 55)             return 'CloudDrizzle';
  if (code === 56 || code === 57)           return 'CloudHail';
  if (code >= 61 && code <= 65)             return 'CloudRain';
  if (code === 66 || code === 67)           return 'CloudHail';
  if (code >= 71 && code <= 77)             return 'Snowflake';
  if (code >= 80 && code <= 82)             return 'CloudRain';
  if (code >= 85 && code <= 86)             return 'Snowflake';
  if (code >= 95 && code <= 99)             return 'CloudLightning';
  return 'Sun';
}

interface WeatherIconProps {
  code: number;
  isNight?: boolean;
  size?: number;
}

export function WeatherIcon({ code, isNight = false, size = 24 }: WeatherIconProps) {
  const name = codeToIconName(code, isNight);
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 100 100"
      style={{ display: 'inline-block', verticalAlign: 'middle', flexShrink: 0 }}
    >
      <path fill={COLORS[name]} fillRule="evenodd" clipRule="evenodd" d={PATHS[name]} />
    </svg>
  );
}
```

- [ ] **Step 2: TypeScript エラーがないことを確認する**

```bash
npx tsc --noEmit
```

期待: エラーなし（警告のみの場合も OK）

- [ ] **Step 3: コミット**

```bash
git add src/components/weather/WeatherIcon.tsx
git commit -m "feat: add WeatherIcon component with Climacons SVG path data"
```

---

## Task 2: HourlyTable の更新

**Files:**
- Modify: `src/components/weather/HourlyTable.tsx`

- [ ] **Step 1: import 行を修正する**

[src/components/weather/HourlyTable.tsx:1-4](src/components/weather/HourlyTable.tsx#L1-L4) の先頭を修正する。

変更前:
```tsx
import React, { useEffect, type CSSProperties, type RefObject } from 'react';
import { Sunrise, Sunset } from 'lucide-react';
import type { HourlyForecast, DailyForecastData } from '../../api/forecast';
import { weatherCodeToEmoji, RISK_BADGES, type RiskType } from '../../lib/riskDetection';
```

変更後:
```tsx
import { useEffect, type CSSProperties, type RefObject } from 'react';
import { Sunrise, Sunset } from 'lucide-react';
import type { HourlyForecast, DailyForecastData } from '../../api/forecast';
import { RISK_BADGES, type RiskType } from '../../lib/riskDetection';
import { WeatherIcon } from './WeatherIcon';
```

- [ ] **Step 2: `nightWeatherNode` 関数を削除する**

[src/components/weather/HourlyTable.tsx:68-78](src/components/weather/HourlyTable.tsx#L68-L78) の以下のブロックをまるごと削除する:

```tsx
// 夜間天気アイコン: 晴れ→✨、薄曇り→✨+☁️(小)、それ以外は昼と同じ
function nightWeatherNode(code: number): React.ReactNode {
  if (code === 0) return '✨';
  if (code <= 2) return (
    <span style={{ display: 'inline-flex', alignItems: 'center', lineHeight: 1 }}>
      <span>✨</span>
      <span style={{ fontSize: '0.65em', marginLeft: '0.05em' }}>☁️</span>
    </span>
  );
  return weatherCodeToEmoji(code);
}
```

- [ ] **Step 3: 天気行のセルを WeatherIcon に置換する**

[src/components/weather/HourlyTable.tsx](src/components/weather/HourlyTable.tsx) の天気行（`{/* 天気 */}` セクション）の hourly セル部分を置換する。

変更前（`key={`w-${i}`}` の td 要素、hourly エントリ側）:
```tsx
                return (
                  <td key={`w-${i}`} style={{ padding: '0.3rem 0.4rem', textAlign: 'center', minWidth: COL_W, fontSize: '1.5em', lineHeight: 1, color: faded ? '#c0c4cf' : '#4b5563' }}>
                    {isNighttime(entry.data.time)
                      ? nightWeatherNode(entry.data.weatherCode)
                      : weatherCodeToEmoji(entry.data.weatherCode)}
                  </td>
                );
```

変更後:
```tsx
                return (
                  <td key={`w-${i}`} style={{ padding: '0.2rem 0.1rem', textAlign: 'center', minWidth: COL_W, verticalAlign: 'middle', opacity: faded ? 0.35 : 1 }}>
                    <WeatherIcon code={entry.data.weatherCode} isNight={isNighttime(entry.data.time)} size={24} />
                  </td>
                );
```

- [ ] **Step 4: TypeScript エラーがないことを確認する**

```bash
npx tsc --noEmit
```

期待: エラーなし

- [ ] **Step 5: コミット**

```bash
git add src/components/weather/HourlyTable.tsx
git commit -m "feat: replace emoji with WeatherIcon in HourlyTable"
```

---

## Task 3: DailyForecast の更新

**Files:**
- Modify: `src/components/weather/DailyForecast.tsx`

- [ ] **Step 1: import 行を修正する**

[src/components/weather/DailyForecast.tsx:1-4](src/components/weather/DailyForecast.tsx#L1-L4) を修正する。

変更前:
```tsx
import { Fragment, type CSSProperties } from 'react';
import type { DailyForecastData } from '../../api/forecast';
import type { DayRisk } from '../../lib/riskDetection';
import { RISK_BADGES, weatherCodeToEmoji } from '../../lib/riskDetection';
```

変更後:
```tsx
import { Fragment, type CSSProperties } from 'react';
import type { DailyForecastData } from '../../api/forecast';
import type { DayRisk } from '../../lib/riskDetection';
import { RISK_BADGES } from '../../lib/riskDetection';
import { WeatherIcon } from './WeatherIcon';
```

- [ ] **Step 2: 分割表示（days 0–2）の AM セルを置換する**

[src/components/weather/DailyForecast.tsx](src/components/weather/DailyForecast.tsx) の AM セル内の天気アイコン部分を置換する。

変更前:
```tsx
                        <div style={{ fontSize: '2.6rem', lineHeight: 1 }}>
                          {day.amWeatherCode !== null ? weatherCodeToEmoji(day.amWeatherCode) : '—'}
                        </div>
```

変更後:
```tsx
                        <div style={{ lineHeight: 1 }}>
                          {day.amWeatherCode !== null ? <WeatherIcon code={day.amWeatherCode} size={42} /> : '—'}
                        </div>
```

- [ ] **Step 3: 分割表示（days 0–2）の PM セルを置換する**

変更前:
```tsx
                        <div style={{ fontSize: '2.6rem', lineHeight: 1 }}>
                          {day.pmWeatherCode !== null ? weatherCodeToEmoji(day.pmWeatherCode) : '—'}
                        </div>
```

変更後:
```tsx
                        <div style={{ lineHeight: 1 }}>
                          {day.pmWeatherCode !== null ? <WeatherIcon code={day.pmWeatherCode} size={42} /> : '—'}
                        </div>
```

- [ ] **Step 4: 単一表示（days 3+）のセルを置換する**

変更前:
```tsx
                  <td key={day.date} style={singleCell(day, i)}>
                    <div style={{ fontSize: '2.6rem', lineHeight: 1 }}>{weatherCodeToEmoji(day.weatherCode)}</div>
                  </td>
```

変更後:
```tsx
                  <td key={day.date} style={singleCell(day, i)}>
                    <div style={{ lineHeight: 1 }}><WeatherIcon code={day.weatherCode} size={42} /></div>
                  </td>
```

- [ ] **Step 5: TypeScript エラーがないことを確認する**

```bash
npx tsc --noEmit
```

期待: エラーなし

- [ ] **Step 6: コミット**

```bash
git add src/components/weather/DailyForecast.tsx
git commit -m "feat: replace emoji with WeatherIcon in DailyForecast"
```

---

## Task 4: 動作確認

- [ ] **Step 1: 開発サーバーを起動する**

```bash
npm run dev
```

ブラウザで `http://localhost:5173` を開く。

- [ ] **Step 2: 日別予報を確認する**

以下をすべて目視確認する:
- 今日・明日・明後日の AM/PM セルに色付きSVGアイコンが表示される
- 4日目以降の単一セルにも同様にアイコンが表示される
- 絵文字（☀️🌤️など）が残っていないこと

- [ ] **Step 3: 時間別予報を確認する**

以下をすべて目視確認する:
- 時間別テーブルの「天気」行に24pxのSVGアイコンが表示される
- 過去時間の列（cutoff より前）でアイコンが薄く（opacity 0.35）表示される
- 日没・日の出列（Sunrise/Sunset アイコン）に影響がないこと
- 夜間時間帯（日没〜日の出）の晴れ（code 0）が Moon アイコン、薄曇り（code 1–2）が CloudMoon アイコンで表示されること

- [ ] **Step 4: コンソールエラーがないことを確認する**

ブラウザの開発者ツール → Console を開き、エラーがゼロであることを確認する。

期待: エラーなし（"Missing Suspense boundary" 等の既存警告は除く）
